  ...
  while True do
    begin
      ...
      case Opcode of
        ...
        OP_SET_CELL, OP_SET_CELL_PARAM:
          begin
            cellIndex := ReadByte;
            Value := IfThen(Opcode = OP_SET_CELL, Peek(0), Peek(cellIndex));
            // Allocate the cell if it's not there yet
            if fn.Cells.Count <= cellIndex then
              begin
                if Opcode = OP_SET_CELL then
                  fn.Cells.Add(asCell(AllocCell(Value)))
                else
                  fn.Cells.Insert(0, asCell(AllocCell(Value)));
              end
            else
        ...
        OP_MAKE_FUNCTION:
          begin
            ...
            for i := 0 to cellCount - 1 do
              newfn.Cells.Insert(0, asCell(Pop)); //.Add(asCell(Pop));
            Push(fnValue);
          end;
