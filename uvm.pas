...
uses
  SysUtils, uOpcodes, uValue, uArray, uParser, uCompiler, uGlobals, Generics.Collections;
...
{
  lambda(Value: PValue) is NumVal(sqr(asNum(Value[0])))

  translate to

  function(Value: PValue): TValue begin Result := NumVal(sqr(asNum(Value[0]))) end
}
...
var
  Opcode, Op, GlobalIndex, LocalIndex, Count, argsCount, cellIndex,
    cellCount: Byte;
  Operand1, Operand2, Value, fnResult, fnValue: TValue;
  Num1, Num2: Double;
  S1, S2: String;
  Condition: Boolean;
  Address: UInt16;
    ...
    begin
      ...
      case Opcode of
        ...
        OP_CALL:
          begin
            argsCount := ReadByte;
            fnValue := Peek(argsCount); // function itself
            // native functions
            if isNative(fnValue) then
              begin
                // call with pointer to first argument: sp-argsCount
                fnResult := asNative(fnValue).Func(sp - argsCount);
                // Pop args + function
                PopN(argsCount + 1);
        ...
        OP_GET_CELL:
          begin
            writeln('OP_GET_CELL start');
            cellIndex := ReadByte;
            Push(fn.Cells[cellIndex].Value);
            writeln('OP_GET_CELL end');
          end;
        OP_SET_CELL:
          begin
            writeln('OP_SET_CELL start');
            cellIndex := ReadByte;
            Value := Peek(0);
            ...
            writeln('OP_SET_CELL index = ', cellIndex);
            writeln('OP_SET_CELL value = ', ValueToString(Value));
            writeln('OP_SET_CELL count = ', fn.Cells.Count);
            writeln('OP_SET_CELL end');
          end;
        OP_LOAD_CELL:
          begin
            writeln('OP_LOAD_CELL start');
            cellIndex := ReadByte;
            writeln('OP_LOAD_CELL index = ', cellIndex);
            writeln('OP_LOAD_CELL count = ', fn.Cells.Count);
            Push(CellVal(fn.Cells[cellIndex]));
            writeln('OP_LOAD_CELL end');
          end;
        OP_MAKE_FUNCTION:
          begin
            CO := asCode(Pop);
            cellCount := ReadByte;
            fnValue := AllocFunction(CO);
